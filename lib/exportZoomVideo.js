...
export async function exportZoomVideo({
  ...
  const dogImg = good ? null : await loadImageOptional(pickDogMemeUrl());
  ...
  onProgress?.(0, "recording", stinger.label);
  ...
  const boomFrames = good ? 0 : Math.max(1, Math.round(BOOM_SEC * FPS));
  ...
  await new Promise((resolve) => {
    ...
    const tick = () => {
      if (frame < scanFrames) {
        ...
        drawDogLowerHalf(ctx, dogImg);
      }

      onProgress?.(Math.round((frame / totalFrames) * 100), "recording", stinger.label);
      frame += 1;
  ...
  const tailMs = good
    ? 50
    : Math.max(200, Math.round((vineBoomDur || 1.25) * 1000) - Math.round(BOOM_SEC * 1000) + 180);
